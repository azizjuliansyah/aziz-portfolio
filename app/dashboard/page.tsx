...
import { LayoutDashboard, Rocket, Code, Share2, User as UserIcon, Settings as SettingsIcon, Plus } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const quickActions = [
    { label: "Edit Profile", icon: UserIcon, color: "text-primary", bg: "bg-primary/10", path: stats.activeProfileId ? `/dashboard/profile/${stats.activeProfileId}?tab=basic` : "/dashboard/profile" },
    { label: "Manage Skills", icon: Code, color: "text-secondary", bg: "bg-secondary/10", path: stats.activeProfileId ? `/dashboard/profile/${stats.activeProfileId}?tab=skills` : "/dashboard/profile" },
    { label: "Add Project", icon: Plus, color: "text-tertiary", bg: "bg-tertiary/10", path: stats.activeProfileId ? `/dashboard/profile/${stats.activeProfileId}?tab=projects` : "/dashboard/profile" },
    { label: "Social Links", icon: Share2, color: "text-primary", bg: "bg-primary/5", path: stats.activeProfileId ? `/dashboard/profile/${stats.activeProfileId}?tab=social-links` : "/dashboard/profile" },
  ];
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {quickActions.map((action) => (
              ...
              >
                ...
                <span className="text-sm font-medium text-on-surface/80">{action.label}</span>
              </button>
            ))}
          </div>
        </div>
